USING: accessors alien.c-types alien.data arrays colors combinators images.loader kernel
literals locals math math.constants math.functions math.matrices.simd math.vectors math.vectors.simd
multiline namespaces opengl opengl.gl opengl.capabilities opengl.shaders opengl.textures
sequences ui ui.gadgets ui.gadgets.tracks ui.gadgets.worlds ui.pens.solid ui.render ;
! To deploy the vocab add one of the following lines
! USE: images.loader.gdiplus   ! for Windows
! USE: images.loader.gtk       ! for Linux
! then replace "vocab:6-gadget/1.jpg" with "./1.jpg" (for Windows)
...
M:: OpenGL-gadget draw-gadget* ( gadget -- )
       ...
       height c - :> b
       { a b } gadget dim>> { 0 0 } vmax gl-viewport
       ! { 0 0 } vmax is needed because the dimensions of the gadget can be negative ! (the gadget is then not visible)
...
: foo  ( -- track )     ! Our OpenGL-gadget between two Factor's native gadgets
       ...
       gadget new { 400 400 } >>dim COLOR: blue <solid> >>interior 1 track-add
