...
math.vectors.simd multiline namespaces opengl opengl.gl opengl.capabilities opengl.framebuffers
...
! then replace "vocab:7-transparency/1.jpg" with "./1.jpg" (for Windows)
...
M:: rotation-world draw-world* ( world -- )
      world [ pt-matrix>> ] [ angle>> (r-matrix) ] bi m4. :> matrix
...
! previous := current }
    ...
    0 2 [| prev | 1 prev - :> curr curr  ! The bracket [| prev | take a number (not 2, look at line 420) from the stack and calls it 'prev'. The last 'curr' is put on the stack.
